...
def get_factorization(X, grad, phi):
    ...
    W = ((R.permute(0, 2, 1) @ R + torch.eye(2)
         [None, :, :].cuda() * 1) * 1).inverse() @ (R.permute(0, 2, 1) @ H)
...
def delta_shift(C, R):
    ...
    result = C + R - (R.sum(dim=dim_range, keepdim=True)) / \
        (nonzero.sum(dim=dim_range, keepdim=True)) * nonzero
...
class Linear(nn.Linear, AGFProp):
    def AGF(self, cam=None, grad_outputs=None, **kwargs):
        if grad_outputs is None:
            ...
            Y = F.softmax(
                yt * torch.exp(-0.5 * ((Y - yt) / sigma) ** 2), dim=1)
...
class Conv2d(nn.Conv2d, AGFProp):
    ...
    def AGF(self, cam=None, grad_outputs=None, **kwargs):
        ...
        M = F.relu((self.X * grad[0]).mean(dim=1,
                   keepdim=True).expand_as(self.X))
        M = safe_divide(M, minmax_dims(M, 'max'))
        ...
        xabs = self.X.abs()
        wabs = self.weight.abs()
        Zabs = F.conv2d(xabs, wabs, bias=None, stride=self.stride,
                        padding=self.padding) * cam.ne(0).type(cam.type())
        ...
        F_x, F_dx = get_factorization(
            self.X, grad[0], C.mean(dim=1, keepdim=True))
        ...
        if "flat" in kwargs.keys() and kwargs["flat"]:
            ...
            wabs = self.weight.abs()
            Zabs = F.conv2d(xabs, wabs, bias=None, stride=self.stride,
                            padding=self.padding) * cam_nonzero
            S = safe_divide(cam, Zabs)
