...
class BasicBlock(nn.Module):
    ...
    def AGF(self, cam, grad_outputs, **kwargs):
        ...
        (cam, cam2), (grad_outputs, grad_outputs2) = self.add.AGF(
            cam, grad_outputs, **kwargs)

        if self.downsample is not None:
            cam2, grad_outputs2 = self.downsample.AGF(
                cam2, grad_outputs2, flat=True, **kwargs)

        cam, grad_outputs = self.bn2.AGF(cam, grad_outputs, **kwargs)
        cam, grad_outputs = self.conv2.AGF(
            cam, grad_outputs, flat=True, **kwargs)
...
class Bottleneck(nn.Module):
    ...
    def AGF(self, cam, grad_outputs, **kwargs):
        ...
        (cam, cam2), (grad_outputs, grad_outputs2) = self.add.AGF(
            cam, grad_outputs, **kwargs)
        grad_outputs = grad_outputs if type(
            grad_outputs) is tuple else (grad_outputs,)
        grad_outputs2 = grad_outputs2 if type(
            grad_outputs2) is tuple else (grad_outputs2,)
        ...
        cam, grad_outputs = self.conv3.AGF(
            cam, grad_outputs, flat=True, **kwargs)
        ...
        cam, grad_outputs = self.relu1.AGF(cam, grad_outputs, **kwargs)
        cam, grad_outputs = self.bn1.AGF(cam, grad_outputs, **kwargs)
        cam, grad_outputs = self.conv1.AGF(
            cam, grad_outputs, flat=True, **kwargs)
